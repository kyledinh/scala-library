...
object Sudoku {
  ...
  def simulate(grid: Vector[BitSet]): Unit = {
    val neighbors = (0 to 80).toVector.map(Sudoku.neighborsOf)
    Sudoku.printAsGrids(grid)

    var turnCount          = 0
    var maxTurns           = 100
    var previousSolveCount = 0
    var currentSolveCount  = Sudoku.solvedCount(grid)

    while Sudoku.solvedCount(grid) < 81 && turnCount < maxTurns && previousSolveCount != currentSolveCount do {
      previousSolveCount = Sudoku.solvedCount(grid)
      Sudoku.scanAndMark(grid, neighbors)
      turnCount += 1
      currentSolveCount = Sudoku.solvedCount(grid)
      println()
      println(s"Turn count: $turnCount")
      Sudoku.printAsGrids(grid)
    }

    if (Sudoku.solvedCount(grid) == 81) { println(s"SOLVED in $turnCount turns!") }
    if (Sudoku.solvedCount(grid) < 81) { println(s"STOPPED at $turnCount turns!") }
    Sudoku.printAsGrids(grid)
  }
  ...
  def loadFromResource(filename: String): Vector[BitSet] = {
    val start =
      Source.fromResource(filename).getLines.mkString.replaceAll("\\.", "0").trim.split(" +").map(_.toInt).toVector
  ...
  def load(filename: String): Vector[BitSet] = {
    val start =
      Source.fromFile(filename).getLines.mkString.replaceAll("\\.", "0").trim.split(" +").map(_.toInt).toVector
    val grid = start.map { n =>
      if n > 0 then BitSet(n)
      else BitSet(1, 2, 3, 4, 5, 6, 7, 8, 9)
    }
